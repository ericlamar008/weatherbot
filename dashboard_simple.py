...
با یک اسکن سبک/کامل تازه به‌روزرسانی نمی‌کرد (این یک مسئلهٔ Workflow است،
نه این فایل؛ باید بررسی شود کدام Workflow اسکن را واقعاً اجرا و commit/
push می‌کند).

همچنین متاتگ‌های Cache-Control/Pragma/Expires به‌تنهایی برای دور زدن کش
CDN گیت‌هاب‌پیجز (Fastly) کافی نبودند -- این متاتگ‌ها فقط یک پیشنهاد به
مرورگر هستند و CDN اصلاً به آن‌ها توجه نمی‌کند. اصلاح قطعی: یک اسکریپت
کوچک در ابتدای <head> اضافه شد که در همان لحظهٔ بازشدن صفحه، اگر لینک
پارامتر ضدکش (?_r=...) نداشته باشد، صفحه را با یک URL کاملاً جدید و یکتا
دوباره بارگذاری می‌کند. چون URL برای CDN و مرورگر متفاوت به‌نظر می‌رسد،
هر بار واقعاً یک درخواست تازه به سرور زده می‌شود، نه نسخهٔ کش‌شده.
"""
import json
